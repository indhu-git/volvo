...
export class WebActions {
    private url = "https://www.volvocars.com/intl/v/car-safety/a-million-more";
    private ec: ProtractorExpectedConditions = browser.ExpectedConditions;
    private timeOut = 10000;


    /**
         * @description This function is used to do the navigation
         * @param For naviagtion to the respective url;
         */
    async navigationToWebApp() {
    ...
    */
    ...
    protected async jsClick(element: ElementFinder) {
        await browser.executeScript("arguments[0].click();", element);
    }
    protected async scrollTo(element: ElementFinder) {
        let loc = await element.getLocation();
        if (loc) {
            await browser.executeScript(`scroll(0, ${loc.y})`)
        }
        else {
            console.log("Failed to Scroll - ");
        };
    }
    protected async scrollAndClick(element: ElementFinder) {
        let loc = await element
            .getLocation();
        if (loc) {
            await browser.executeScript(`scroll(0, ${loc.y - 100})`)
    ...
    public async assertTitle(text) {
        expect(await browser.getTitle()).toContain(text);
    }
